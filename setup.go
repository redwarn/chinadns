...
func setup(c *caddy.Controller) error {
	...
	if f.Len() > max {
		return plugin.Error("chinadns", fmt.Errorf("more than %d TOs configured: %d", max, f.Len()))
	}
...
func (f *ChinaDNS) OnStartup() (err error) {
...
func (f *ChinaDNS) OnShutdown() error {
	for _, p := range f.proxies {
		p.stop()
	}
	return nil
...
func parseStanza(c *caddy.Controller) (*ChinaDNS, error) {
	f := New()
